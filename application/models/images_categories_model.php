<?php
class Images_categories_model extends MY_Model {
	
	var $table = 'images_categories';

	function get($filters = array()) 	
	{
		$select = "SELECT images_categories.*";
 		$from = " FROM images_categories";
 		$order = " ORDER BY sort";
    ...
    {
    	$CI = & get_instance();
    	$query = "SELECT `images`.`id` FROM `images`  WHERE `id_category`='".$id_category."'";
    	$query_db = $this->db->query($query);
        $results = $query_db->result_array();
    	foreach ($results as $result)
    	{
    		$CI->Images_model->destroy($result['id']);
    	}
    	//delete category
